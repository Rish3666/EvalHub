...
-- OPTIMIZED: Composite indexes for Chat performance, strict membership RLS.
...
-- COMPOSITE INDEX for fast Chat History loading
CREATE INDEX IF NOT EXISTS idx_messages_community_created ON public.messages(community_id, created_at);
...
CREATE POLICY "Users can follow" ON public.community_follows 
  FOR INSERT WITH CHECK (auth.uid() = user_id);
